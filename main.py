...
if __name__ == '__main__':
    ...
    runmode_config_file_path = platform_dir + '/config/runmodeconfig_release.conf'
    logger.info('正在读取运行模式配置')
    if sys.argv[1] == '1':
        run_mode_conf = RunModeConfig(runmode_config_file_path)
    elif sys.argv[1] == '2':
        run_mode_conf = RunModeConfig(runmode_config_file_path)
    ...
    if 1 == run_mode:
        ...
        for testsuite_id in testsuits_id_list:
            ...
            testsuite_conf = other_tools.conver_date_from_testlink(testsuite_conf)
            if '' == testsuite_conf:
    ...
    if 4 != run_mode:
        ...
        report_config_file_path = platform_dir + '/config/report.conf'
        config.read(report_config_file_path, encoding='utf-8')
