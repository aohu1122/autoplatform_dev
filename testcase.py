...
from testcases.test_LoginLogout import *

class TestCase:
    def __init__(self, testcase_id, testcase_name, steps, active_status, testproject, testcase_summary, testplan="default"):
        self.testcase_id = testcase_id
        self.testcase_name = testcase_name
        self.steps = steps
        self.active_status = active_status # 用例是否禁用 1 - 活动 0 - 禁用
        self.testproject = testproject
        self.test_summary = testcase_summary
        self.testplan = testplan

    #def testsuite_addcases(self):
    #def string2func(self, string1, string2):

    def find_autocase(self, testcase_summary):
        platform_dir = other_tools.get_platform_dir()
        testdir = platform_dir + '/testcases/'
        logger.info('sion::Will find test cases: %s' % testcase_summary)
        discover = unittest.defaultTestLoader.discover(testdir, pattern=testcase_summary)
        #testsuite1 = unittest.TestSuite()
        #testsuite1.addTest(TestLogin("test_LoginLogout"))
        logger.info('sion:discover testcases number:::%s' % discover.countTestCases())
        ...
        with open(report_name, 'wb') as f:
            ...
            try:
                ...
                logger.error('test case error %s' % e)
        f.close()
